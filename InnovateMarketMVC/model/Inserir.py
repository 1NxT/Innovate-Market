...
class Inserir():
    ...
    def salvarProdutoCaixa(self, valores):
        self.__cursor.execute("INSERT INTO caixaCompras (CodigoCompra, Nome_Produto, Qtd, CodigoProduto) VALUES (?, ?, ?, ?)", (valores.get('CodigoCompra'), valores.get('nomeProduto'), valores.get('Qtd'), valores.get('CodigoProduto')))
        self.__cursor.execute("commit;")
        DB().closeCursor()
